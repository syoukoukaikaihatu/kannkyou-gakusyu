const KEY='wme_v40_state';
const OLD_KEY='wme_v31_state';
const VERSION='4.0.0';
const curriculum=[
...
function freshState(){return {version:VERSION,quickInbox:[],knowledgeLevels:{},bridge:[],roadmapProgress:{},monthlyReviews:{},completed:[],lessonNotes:{},actions:[
 ...
 {id:crypto.randomUUID(),topic:'自治体定修・緊急受入れを地域BCPの観点でも整理する',status:'未確認',decision:'',owner:''}],knowledge:starterKnowledge,meetingNotes:'',currentLesson:'01',growth:[],factoryVoice:[],executiveIssues:[],agriCompleted:[],agriNotes:{},agriCurrent:'A01',agriKnowledgeLevels:{},synergyIdeas:[],synergyProgress:{},lastUpdated:new Date().toISOString()};}
let state=load(); state.quickInbox=state.quickInbox||[]; state.knowledgeLevels=state.knowledgeLevels||{}; state.bridge=state.bridge||[]; state.agriCompleted=state.agriCompleted||[]; state.agriNotes=state.agriNotes||{}; state.agriKnowledgeLevels=state.agriKnowledgeLevels||{}; state.synergyIdeas=state.synergyIdeas||[]; state.synergyProgress=state.synergyProgress||{}; let currentPage='home';
...
const navItems=[['home','HOME｜全体進捗'],['quick','QUICK CAPTURE｜30秒メモ'],['inbox','INBOX｜未整理'],['roadmap','WASTE ROADMAP｜廃棄物12か月'],['learning','WASTE LEARNING｜廃棄物研修'],['agri','AGRI LEARNING｜農業研修'],['agriMap','AGRI KNOWLEDGE｜農業理解度'],['synergy','CIRCULAR PROJECT｜余熱×農業'],['growth','MY GROWTH｜成長記録'],['knowledge','WASTE KNOWLEDGE｜廃棄物知識'],['facility','FACILITY｜設備'],['license','LICENSE｜資格・人材'],['government','GOVERNMENT｜行政・自治体'],['bridge','MANAGEMENT BRIDGE｜経営への論点'],['alignment','ALIGNMENT｜共通認識'],['actions','ACTION｜課題管理'],['meeting','MEETING｜経営共有']]
...
function render(page,arg){const c=document.getElementById('content'),title=document.getElementById('pageTitle');const labels=Object.fromEntries(navItems);title.textContent=labels[page]||'研修'; if(page==='home')home(c); if(page==='quick')quickCapture(c); if(page==='inbox')inbox(c); if(page==='roadmap')roadmap(c); if(page==='growth')growth(c); if(page==='learning')learning(c); if(page==='lesson')lesson(c,arg||state.currentLesson); if(page==='agri')agriLearning(c); if(page==='agriLesson')agriLesson(c,arg||state.agriCurrent); if(page==='agriMap')agriMap(c); if(page==='synergy')synergy(c); if(page==='knowledge')knowledge(c); if(page==='facility')facility(c); if(page==='license')license(c); if(page==='government')government(c); if(page==='management')management(c); if(page==='alignment')alignment(c); if(page==='actions')actions(c); if(page==='meeting')meeting(c);}

const agriCurriculum=[
 {stage:'Stage A1｜農業・市場の全体像',lessons:[['A01','日本農業の全体像','市場規模・主要品目・プレイヤー・構造変化を説明できる。'],['A02','農業のサプライチェーン','生産から集荷・加工・流通・小売・外食までの商流を理解する。'],['A03','農業経営体・法人化・大規模化','担い手減少と法人化・農地集約が事業機会にどうつながるか考える。'],['A04','千葉県・成田周辺の農業','地域の主要品目・産地・農業法人・物流条件を調査する。']]},
 {stage:'Stage A2｜品目別ビジネス',lessons:[['A05','米・水田作','収量・単価・機械・土地利用から収益構造を理解する。'],['A06','露地野菜','作型・労働・天候・市場価格リスクを理解する。'],['A07','施設園芸','ハウス・暖房・環境制御・周年栽培の経営構造を理解する。'],['A08','果樹・畜産・その他','主要分野の投資・回収期間・労働特性を比較する。']]},
 {stage:'Stage A3｜農業経営',lessons:[['A09','農業のPL・原価','反収、歩留まり、単価、人件費、肥料・燃料・設備費を分解する。'],['A10','販売・契約・ブランド','市場出荷、契約栽培、直販、EC、加工の違いを理解する。'],['A11','農地・法人・制度','農地法、農業法人、参入時の基本的な制度を整理する。'],['A12','人材・スマート農業','人手不足、外国人材、自動化、ロボット、環境制御を学ぶ。']]},
 {stage:'Stage A4｜事業化・循環型農業',lessons:[['A13','農業政策・補助制度','国・県の政策方向と補助制度を事業判断と切り分けて理解する。'],['A14','農業参入モデル比較','自社生産、共同事業、熱供給、施設賃貸、農業支援サービスを比較する。'],['A15','廃棄物処理施設の余熱利用','利用可能熱量、温度、距離、需要時間帯、設備投資を整理する。'],['A16','余熱利用型施設園芸FS','作物・面積・熱需要・売上・CAPEX/OPEX・法制度を統合し事業性を評価する。']]}
];
function allAgriLessons(){return agriCurriculum.flatMap(s=>s.lessons.map(x=>({stage:s.stage,no:x[0],title:x[1],goal:x[2]})))}
function agriLessonByNo(n){return allAgriLessons().find(x=>x.no===n)}
function agriPct(){return Math.round((state.agriCompleted||[]).length/allAgriLessons().length*100)}
function agriLearning(c){const lessons=allAgriLessons();c.innerHTML=`<div class="hero"><span class="badge blue">AGRICULTURE EXECUTIVE LEARNING</span><h2>農業を「作り方」ではなく「産業・経営・事業機会」から学ぶ</h2><p class="muted">市場→品目→収益構造→制度→スマート農業→余熱利用型施設園芸へ進みます。</p></div><div class="section-head"><div><h2>全16回 基礎・事業化課程</h2><div class="muted">廃棄物研修とは独立して進捗を保存します。</div></div><strong>${state.agriCompleted.length}/16 完了</strong></div><div class="progress" style="margin-bottom:18px"><i style="width:${agriPct()}%"></i></div><div class="curriculum">${agriCurriculum.map(s=>`<div class="card"><div class="stage-title">${s.stage}</div>${s.lessons.map(l=>`<div class="lesson" data-ano="${l[0]}"><div class="lesson-no">${l[0]}</div><div><div class="lesson-title">${l[1]}</div><div class="small muted">${l[2]}</div></div><span class="badge ${state.agriCompleted.includes(l[0])?'ok':''}">${state.agriCompleted.includes(l[0])?'完了':state.agriNotes[l[0]]?'進行中':'未実施'}</span></div>`).join('')}</div>`).join('')}</div>`;document.querySelectorAll('[data-ano]').forEach(x=>x.onclick=()=>go('agriLesson',x.dataset.ano));}
function agriLesson(c,no){const l=agriLessonByNo(no);if(!l)return go('agri');state.agriCurrent=no;save();const n=state.agriNotes[no]||{};c.innerHTML=`<div class="hero"><div class="badge blue">${l.stage}</div><h2>${l.no}｜${l.title}</h2><p class="muted">今日のゴール：${l.goal}</p></div><div class="card"><h3>研修記録</h3><div class="form-grid"><div class="field full"><label>① 今日わかったこと｜FACT / KNOWLEDGE</label><textarea id="aFact">${esc(n.fact||'')}</textarea></div><div class="field"><label>② 自分の仮説・考え｜HYPOTHESIS</label><textarea id="aHyp">${esc(n.hypothesis||'')}</textarea></div><div class="field"><label>③ 収益構造・経営への示唆</label><textarea id="aBiz">${esc(n.business||'')}</textarea></div><div class="field"><label>④ 廃棄物・自社との接点｜OPPORTUNITY</label><textarea id="aOpp">${esc(n.opportunity||'')}</textarea></div><div class="field"><label>⑤ 次に確認すること｜ACTION</label><textarea id="aAction">${esc(n.action||'')}</textarea></div><div class="field full"><label>根拠URL・資料名・確認日</label><textarea id="aSource">${esc(n.source||'')}</textarea></div></div><div class="bridge-choice"><label><input type="checkbox" id="aSynergy" ${n.synergy?'checked':''}> 余熱×農業／新規事業の検討候補にする</label></div><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:14px"><button class="btn" id="saveAgri">記録を保存</button><button class="btn secondary" id="completeAgri">${state.agriCompleted.includes(no)?'完了を取り消す':'この回を完了'}</button></div></div>`;function collect(){const o={fact:aFact.value,hypothesis:aHyp.value,business:aBiz.value,opportunity:aOpp.value,action:aAction.value,source:aSource.value,synergy:aSynergy.checked};state.agriNotes[no]=o;save();return o}saveAgri.onclick=()=>{const o=collect();if(o.synergy&&o.opportunity.trim()){state.synergyIdeas.unshift({id:crypto.randomUUID(),date:new Date().toISOString().slice(0,10),title:`${no}｜${l.title}`,idea:o.opportunity,status:'仮説',memo:o.business});save();alert('保存し、CIRCULAR PROJECTにも追加しました')}else alert('保存しました')};completeAgri.onclick=()=>{collect();state.agriCompleted=state.agriCompleted.includes(no)?state.agriCompleted.filter(x=>x!==no):[...state.agriCompleted,no];save();render('agriLesson',no)};}
function agriMap(c){const defaults=['日本農業の市場構造','農業サプライチェーン','農業法人・大規模化','千葉県・成田周辺','米・水田作','露地野菜','施設園芸','農業PL・原価','販売・流通','農地・法制度','人材・スマート農業','余熱利用型農業'];defaults.forEach(t=>{if(!state.agriKnowledgeLevels[t])state.agriKnowledgeLevels[t]={title:t,level:0,note:''}});const rows=Object.values(state.agriKnowledgeLevels);c.innerHTML=`<div class="hero"><span class="badge blue">AGRI KNOWLEDGE MAP</span><h2>農業の理解度をLv0〜Lv5で可視化</h2><p class="muted">Lv0 知らない → Lv1 聞いたことがある → Lv2 説明できる → Lv3 事業に当てはめられる → Lv4 課題・機会を発見できる → Lv5 投資判断の論点を整理できる</p></div><div class="table-wrap"><table><thead><tr><th>テーマ</th><th>理解度</th><th>メモ</th></tr></thead><tbody>${rows.map(x=>`<tr><td><b>${esc(x.title)}</b></td><td><select data-akl="${escAttr(x.title)}">${[0,1,2,3,4,5].map(v=>`<option value="${v}" ${Number(x.level)===v?'selected':''}>Lv.${v}</option>`).join('')}</select></td><td><input data-akn="${escAttr(x.title)}" value="${escAttr(x.note||'')}"></td></tr>`).join('')}</tbody></table></div>`;document.querySelectorAll('[data-akl]').forEach(el=>el.onchange=()=>{state.agriKnowledgeLevels[el.dataset.akl].level=Number(el.value);save()});document.querySelectorAll('[data-akn]').forEach(el=>el.onchange=()=>{state.agriKnowledgeLevels[el.dataset.akn].note=el.value;save()});}
const synergySteps=[['S1','余熱の供給条件','利用可能熱量・温度・時間帯・季節変動を確認'],['S2','立地・導管','工場から候補地までの距離、熱輸送方法、ロスを確認'],['S3','作物候補','熱需要・単価・栽培難易度・販路から候補を比較'],['S4','施設規模','温室面積・必要熱量・生産量を仮置き'],['S5','事業スキーム','自社栽培／農業法人連携／熱供給／施設賃貸を比較'],['S6','法制度','農地・都市計画・建築・環境・許認可を確認'],['S7','投資採算','CAPEX・OPEX・売上・粗利・回収年数を試算'],['S8','実証・意思決定','小規模実証、Go/No-Go条件、次段階を決定']];
function synergy(c){const done=synergySteps.filter(x=>(state.synergyProgress[x[0]]||{}).status==='完了').length;c.innerHTML=`<div class="hero"><span class="badge blue">CIRCULAR BUSINESS PROJECT</span><h2>廃棄物処理施設の余熱 × 農業</h2><p class="muted">学習で得た知識を、実際の新規事業FSへつなげる専用エリアです。結論を急がず、技術・市場・制度・採算を順番に確認します。</p></div><div class="grid cols-3"><div class="card"><div class="small muted">FS進捗</div><div class="metric">${done}/8</div></div><div class="card"><div class="small muted">事業アイデア</div><div class="metric">${state.synergyIdeas.length}</div></div><div class="card"><div class="small muted">農業学習</div><div class="metric">${agriPct()}%</div></div></div><div class="section-head" style="margin-top:18px"><div><h2>事業化ステップ</h2></div></div>${synergySteps.map(x=>{const p=state.synergyProgress[x[0]]||{status:'未着手',memo:''};return `<div class="card" style="margin-bottom:12px"><div class="section-head"><div><span class="badge ${p.status==='完了'?'ok':p.status==='確認中'?'warn':'blue'}">${p.status}</span><h3 style="margin-top:8px">${x[0]}｜${x[1]}</h3><div class="muted">${x[2]}</div></div><select data-sp="${x[0]}"><option ${p.status==='未着手'?'selected':''}>未着手</option><option ${p.status==='確認中'?'selected':''}>確認中</option><option ${p.status==='完了'?'selected':''}>完了</option></select></div><textarea data-sm="${x[0]}" style="width:100%;min-height:80px" placeholder="確認した事実・数値・次の論点">${esc(p.memo||'')}</textarea></div>`}).join('')}<div class="card"><div class="section-head"><div><h2>学習から生まれた事業アイデア</h2></div><button class="btn" id="addSynergy">アイデア追加</button></div>${state.synergyIdeas.map(x=>`<div class="list-row"><span class="dot blue"></span><div><b>${esc(x.title)}</b><div class="small muted">${esc(x.idea)} ${x.memo?'｜'+esc(x.memo):''}</div></div><span class="badge">${esc(x.status)}</span></div>`).join('')||'<div class="empty">まだありません。農業研修から候補を送れます。</div>'}</div>`;document.querySelectorAll('[data-sp]').forEach(el=>el.onchange=()=>{state.synergyProgress[el.dataset.sp]=state.synergyProgress[el.dataset.sp]||{};state.synergyProgress[el.dataset.sp].status=el.value;save();render('synergy')});document.querySelectorAll('[data-sm]').forEach(el=>el.onblur=()=>{state.synergyProgress[el.dataset.sm]=state.synergyProgress[el.dataset.sm]||{status:'未着手'};state.synergyProgress[el.dataset.sm].memo=el.value;save()});addSynergy.onclick=()=>{const idea=prompt('事業アイデア・検証したい仮説');if(!idea)return;state.synergyIdeas.unshift({id:crypto.randomUUID(),date:new Date().toISOString().slice(0,10),title:'手動追加',idea,status:'仮説',memo:''});save();render('synergy')}}
...
function home(c){const next=allLessons().find(l=>!state.completed.includes(l.no))||allLessons()[23];const nextAgri=allAgriLessons().find(l=>!state.agriCompleted.includes(l.no))||allAgriLessons()[15];const inboxCount=(state.quickInbox||[]).filter(x=>!x.sorted).length;const bridgeCount=(state.bridge||[]).filter(x=>x.status!=='送付済').length;const doneMonths=yearlyRoadmap.filter(r=>(state.roadmapProgress[r[0]]||{}).status==='完了').length;const synDone=synergySteps.filter(x=>(state.synergyProgress[x[0]]||{}).status==='完了').length;c.innerHTML=`<div class="hero"><span class="badge blue">CIRCULAR BUSINESS LEARNING OS｜v4.0</span><h2>廃棄物を深く理解し、農業を学び、余熱利用の事業可能性へつなげる。</h2><p class="muted">専門知識・現場事実・農業市場・新規事業FSを一つのサイトで管理します。経営判断そのものはMANAGEMENT BRIDGEで整理します。</p></div><div class="grid cols-3"><div class="card"><span class="badge">WASTE</span><h3>廃棄物・工場経営研修</h3><div class="metric">${state.completed.length}/24</div><div class="progress"><i style="width:${pct()}%"></i></div><p class="small muted">次回 ${next.no}｜${esc(next.title)}</p><button class="btn" onclick="go('lesson','${next.no}')">続きを学ぶ</button></div><div class="card"><span class="badge">AGRICULTURE</span><h3>農業・農業経営研修</h3><div class="metric">${state.agriCompleted.length}/16</div><div class="progress"><i style="width:${agriPct()}%"></i></div><p class="small muted">次回 ${nextAgri.no}｜${esc(nextAgri.title)}</p><button class="btn" onclick="go('agriLesson','${nextAgri.no}')">続きを学ぶ</button></div><div class="card"><span class="badge">CIRCULAR PROJECT</span><h3>余熱 × 農業 FS</h3><div class="metric">${synDone}/8</div><div class="progress"><i style="width:${synDone/8*100}%"></i></div><p class="small muted">技術・市場・制度・採算を統合</p><button class="btn" onclick="go('synergy')">事業検討を開く</button></div></div><div class="quick-grid"><button class="quick-card primary" onclick="go('quick')"><span>01</span><b>30秒メモ</b><small>現場・農業・事業アイデアをまず残す</small></button><button class="quick-card" onclick="go('inbox')"><span>02</span><b>INBOXを整理</b><small>未整理 ${inboxCount}件</small></button><button class="quick-card" onclick="go('bridge')"><span>03</span><b>経営論点を見る</b><small>未送付 ${bridgeCount}件</small></button></div><div class="grid cols-2" style="margin-top:16px"><div class="card"><h3>3つの役割</h3><p><b>廃棄物：</b>工場・法令・設備・操業を理解</p><p><b>農業：</b>市場・品目・収益・制度を理解</p><p><b>余熱×農業：</b>両方の知識を実際の事業性調査へ接続</p></div><div class="card"><h3>今月の見方</h3><p class="muted">廃棄物ロードマップ ${doneMonths}/12完了。農業は全16回を独立進行。学習量ではなく「説明できる・自社に当てはめられる・事業判断の論点を整理できる」を成長基準にします。</p></div></div>`;}
...
document.getElementById('exportBtn').onclick=()=>{const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`circular-business-learning-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href)};
