...
<div class="card course-card agri"><div class="small muted">農業</div><div class="metric">${pct('agri')}%</div><div class="progress"><i style="width:${pct('agri')}%"></i></div><h3 style="margin-top:14px">次回 ${next('agri')[0]}｜${next('agri')[1]}</h3><button class="btn big-action" onclick="go('lesson','${next('agri')[0]}')">農業を学ぶ</button></div>
...
<div class="grid cols-2" style="margin-top:16px"><div class="card"><h3>全体進捗</h3><div class="metric">${done}/${total}</div><p class="muted">学習回数だけでなく、学習から見えた「会社の事実・課題候補・未確認事項」も自動で蓄積します。</p></div><div class="card"><h3>直近の学習</h3>${recent(4)}</div></div>`}
